...
describe "uploading a file" do
  ...
  let(:good_tab_file)    { Rails.root.join('spec/fixtures/good_data.tab') }
  let(:bad_tab_file)     { Rails.root.join('spec/fixtures/bad_data.tab') }
  let(:invalid_tab_file) { Rails.root.join('spec/fixtures/rails.png') }

  context "anonymous" do
    before do
      # Stub out authentication
      ImportsController.any_instance.stub(:signed_in?) { false }
    end

    it "requires a login" do
      visit import_path
      page.should have_content "must be logged in"
      page.should_not have_xpath "//form[@action='/import']"
    end
  end
  ...
  context "authenticated" do
    before do
      # Stub out authentication
      ImportsController.any_instance.stub(:signed_in?) { true }
    end
